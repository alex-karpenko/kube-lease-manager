...
/// but may be used as a skeleton for a multi-pod controller or resource watcher.
///
/// `LeaseManager` is used to run ensure a single instance of the logger is running at any moment of time.
/// If it currently doesn't hold the lock - it shuts down the logger (if it's running).
...
async fn main() -> Result<(), anyhow::Error> {
    ...
    loop {
        ...
        if is_leader && logger_task.is_none() {
            // if we hold the lock but the logger isn't running - run it
            info!("got leader lock, running new job logger task");
            tx.send_replace(false); // set the shutdown state to false
        ...
        tokio::select! {
            _ = lease_channel.changed() => {
                debug!("lease lock state changed"); // continue the loop
            }
    ...
    // stop a logger task if it's running
...
/// Spawns a detached Tokio task which watches on all Jobs in the cluster
/// and logs any changes on INFO level
///
/// It finishes as soon as the `true` value is posted to the shutdown channel.
///
...
fn watch_jobs(client: Client, mut shutdown: Receiver<bool>) -> JoinHandle<()> {
        ...
        tokio::select! {
            ...
            _ = async move {
                while shutdown.changed().await.is_ok() {
                    // If the shutdown state was changed to true - break the loop,
                    // so the `select` block finishes
    ...
    // Run a detached watcher task and return its handler
    tokio::spawn(watcher)
}
